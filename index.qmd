...
Data <-
  ...
  as.data.frame() |>
  # Remove columns.
  select(-c("Used","Duplicate","Not_Digitized")) # 2178 occurrences with species names.
...
clean_spatial_data |>
  ...
  geom_col() +
  geom_hline(yintercept = c(10,15,20,25,30)) + # Minimum number of occurrences.
  theme_bw() +
...
# At a minimum occurrences of 15, the interval with least number of cells is the Thanetian (Ncells = 5)
...
# Assign cells to coordinate data and remove occurrence below the minimum thresh-hold.
...
clean_spatial_data <- bind_rows(campanian_cells,maastrich_cells,daniansel_cells,thanetian_cells) # 2056 specimens in total.
...
pathToPolygons  <- file.path(tempdir(), "PALEOMAP_PlatePolygons.gpml") # static plate polygons
...
jaccard_similarity <- function(x) {
  ...
  jaccard_similarity_table <- data.frame(cell_x = character(), cell_y = character(),
  ...
  for (i in 1:length(unique_cells)) {
    ...
    for (j in 1:length(unique_cells)) {
      ...
      # Combine results.
      jaccard_similarity_table <- rbind(jaccard_similarity_table, data.frame(cell_x = cell_x, cell_y = cell_y, 
...
# Campanian.
cmp_jaccard <- jaccard_similarity(x = combined_cam)
# Maastrichtian.
mas_jaccard <- jaccard_similarity(combined_mas)
# Danian/Selandian.
dan_jaccard <- jaccard_similarity(combined_dan)
# Thanetian.
tha_jaccard <- jaccard_similarity(combined_tha)
...
# Combine and label summary data
summary_data <- bind_rows(
  summary_statistics(x = cmp_binned) |> mutate(Stage = "Campanian"),
  summary_statistics(x = mas_binned) |> mutate(Stage = "Maastrichtian"),
  summary_statistics(x = dan_binned) |> mutate(Stage = "Danian"),
  summary_statistics(x = tha_binned) |> mutate(Stage = "Thanetian")
) |> 
  mutate(Stage = factor(Stage, levels = c("Campanian", "Maastrichtian", "Danian", "Thanetian")))

# Plot
ggplot(summary_data, aes(x = cutdist, y = avg, group = Stage, colour = Stage, fill = Stage)) +
  # Error bars
  geom_errorbar(
    aes(ymin = second, ymax = first),
    width = 0.05,
    linewidth = 0.8,
  ...
  # Labels and scales
  scale_size_continuous(name = "Number of cell-pair comparisons", breaks = c(5, 10, 15, 20, 25, 30)) +
  labs(
    x = "Great Circle Distance (km)",
    y = "Jaccard Similarity",
    title = "Distance Decay of Similarity",
    subtitle = "Subsampled by cells and occurrences",
    colour = "Stage"
  ) +
