...
rpkg <- c("dplyr ggplot2 readr boot divvy terra divDyn conflicted rgplates sf colorspace rgl RColorBrewer
           icosa tidyr tibble purrr downloadthis ggpubr openxlsx igraph Matrix RColorBrewer
           betapart rnaturalearth dispeRse chronosphere ncdf4")
...
Data <-
  ...
  as.data.frame() |>
  # Remove columns.
  select(-c("Used","Duplicate","Not_Digitized","Age.2","Age.PD")) # 2444 occurrences with species names.
```
...
clean_spatial_data <- bind_rows(campanian_cells,maastrich_cells,daniansel_cells,thanetian_cells) # 2365 specimens in total.
# Create unique identifier for each cell.
clean_spatial_data <-
  data.frame(unique(clean_spatial_data$cell)) |> # 31 unique cells.
  setNames(nm = "cell") |> 
...
topo_camp <- basemap[[19]] #80.3 Ma       class: spatraster
coast_camp <- coastlines[[15]] # 80 Ma    class: sfc_POLYGON"
...
set.seed(1)
boot_campanian <- 
  boot_age(x = campanian_split,n_boot = 999,n_strata = 8,n_per_stratum = 10) |> 
  combine_boot_lists() # retains the bootstrap structure with each replicate.

# Maastrichtian.
set.seed(2)
boot_maastrichtian <- 
  boot_age(x = maastrichtian_split,n_boot = 999,n_strata = 8,n_per_stratum = 10) |> 
  combine_boot_lists()

# Danian-Selandian.
set.seed(3)
boot_danian <- 
  boot_age(x = danian_split,n_boot = 999,n_strata = 8,n_per_stratum = 10) |> 
  combine_boot_lists()

# Thanetian.
set.seed(4)
boot_thanetian <- 
  boot_age(x = thanetian_split,n_boot = 999,n_strata = 8,n_per_stratum = 10) |> 
  combine_boot_lists()
...
For a given taxonomic-unit (i.e.,genus or species) we caluclate all pairwise cell similarities using the Jaccard index for each bootstrap replicate.
...
matrix_results <- function(x) {
  ...
  # GCD.
  mutate(gcd = gcd_slc(long1 = x_long,lat1 = x_lat,long2 = y_long,lat2 = y_lat)) |> 
  as.data.frame()
}
...
# mean_jaccard <- function(x) {
#   bind_rows(x, .id = "replicate_id") |>  # Keep which replicate it came from
#     group_by(replicate_id, cell_x, cell_y) |> 
#     summarise(avg_jaccard = mean(jaccard_similarity), .groups = "drop")
# }
...
#     rename("x.cell" = "x", "y.cell" = "y") |> 
#     left_join(
#       y |> rename("x.cell" = "cell_x", "y.cell" = "cell_y"), 
#       by = c("x.cell", "y.cell")
#     )
# }
...
# Plot.
  ...
  geom_errorbar(aes(ymin = first, ymax = second), width = 0.05, linewidth = 1, position = position_dodge(width = 0.3)) + 
...
cmp_long <- map2_dfr(
  cmp_jaccard_species,
  seq_along(cmp_jaccard_species), # replicate index
  ~ mutate(.x, replicate = .y)
)

cmp_long <- cmp_long %>%
  left_join(cmp_matrix, by = c("cell_x" = "x", "cell_y" = "y"))

cmp_long <- cmp_long %>%
  mutate(cutdist = bin_distance(gcd))
...
## Function to go from bootstrap list -> summary with CIs
process_bootstrap <- function(jaccard_list, dist_matrix) {
  
  # 1. Long format with replicate index
  long_df <- map2_dfr(
    jaccard_list,
    seq_along(jaccard_list),
    ~ mutate(.x, replicate = .y)
  )
  
  # 2. Merge distances
  long_df <- long_df %>%
    left_join(dist_matrix, by = c("cell_x" = "x", "cell_y" = "y"))
  
  # 3. Add distance bins
  long_df <- add_cutdist(long_df, new_col = "gcd")
  
  # 4. Mean jaccard per replicate per bin
  bin_rep <- long_df %>%
    group_by(replicate, cutdist) %>%
    summarise(
  ...
  # 5. Across-replicates mean and bootstrap CIs
  bin_summary <- bin_rep %>%
    group_by(cutdist) %>%
    summarise(
      mean_jaccard = mean(mean_jaccard, na.rm = TRUE),
      lower_CI = quantile(mean_jaccard, 0.025, na.rm = TRUE),
      upper_CI = quantile(mean_jaccard, 0.975, na.rm = TRUE),
      mean_n_pairs = mean(n_pairs),
      .groups = "drop"
    )
...
## Example usage for Campanian
cmp_bin_summary <- process_bootstrap(cmp_jaccard_species, cmp_matrix)

## Plot
ggplot(cmp_bin_summary, aes(x = cutdist, y = mean_jaccard)) +
  geom_point(aes(size = mean_n_pairs)) +
