...
rpkg <- c("dplyr ggplot2 readr boot divvy terra divDyn conflicted rgplates sf colorspace rgl
           icosa tidyr tibble purrr downloadthis ggpubr openxlsx igraph Matrix")
...
Data <-
  ...
  as.data.frame() |>
  # Remove columns.
  select(-c("Used","Duplicate","Not_Digitized")) # 2206 occurrences with species names.
```
...
spatial_data |>
  ...
  mutate(source_type = case_when(Source == "WAM Collection" ~ "Museum collection",T ~ source_type)) |> 
  group_by(Source,Age.3,source_type) |> 
  count() |> 
...
clean_spatial_data <- bind_rows(campanian_cells,maastrich_cells,daniansel_cells,thanetian_cells) # 2071 specimens in total.
# Create unique identifier for each cell.
clean_spatial_data <-
  data.frame(unique(clean_spatial_data$cell)) |> # 24 unique cells.
  setNames(nm = "cell") |> 
