...
Data <-
  ...
  as.data.frame() |>
  # Remove columns.
  select(-c("Used","Duplicate","Not_Digitized","Age.2","Age.PD")) # 2381 occurrences with species names.
```
...
clean_spatial_data <- bind_rows(campanian_cells,maastrich_cells,daniansel_cells,thanetian_cells) # 2302 specimens in total.
# Create unique identifier for each cell.
clean_spatial_data <-
  data.frame(unique(clean_spatial_data$cell)) |> # 29 unique cells.
  setNames(nm = "cell") |> 
...
co_occurrence_matrix <- function(data,age, taxon = NULL) {
  data <- data |> filter(Age.3 == age)
  data |>
...
cmp_bc <- co_occurrence_matrix(data = clean_spatial_data,age = "Campanian") |> 
  dispeRse::BC(tree = NULL,jackknife = TRUE,
               resample.replicates = 9999)

maa_bc <- co_occurrence_matrix(data = clean_spatial_data,age = "Maastrichtian") |> 
  dispeRse::BC(tree = NULL,jackknife = TRUE,
               resample.replicates = 9999)

dan_bc <- co_occurrence_matrix(data = clean_spatial_data,age = "DanSelCombined") |> 
  dispeRse::BC(tree = NULL,jackknife = TRUE,
               resample.replicates = 9999)

tha_bc <- co_occurrence_matrix(data = clean_spatial_data,age = "Thanetian") |>
  dispeRse::BC(tree = NULL,jackknife = TRUE,
               resample.replicates = 9999)
