...
```{r}
rpkg <- c("dplyr ggplot2 readr boot divvy terra divDyn conflicted CoordinateCleaner 
          rgplates icosa tidyr tibble purrr downloadthis ggpubr openxlsx")
...
# 51 occurences from Limhamn quarry that needs to be ID.
...
Data <- Data |> filter(!Species %in% remove_obs) # Ask Mikael to provide IDs for all Limnhamn specimens. But remove for now.

# Final subset (remove occurences with no species identification)
Data <- 
  Data |> 
  ...
  as.data.frame() |>
  # Remove columns.
  select(-c("Used","Duplicate","Not_Digitized")) # 2136 occurences with species names.
...
```{r}
# Number of occurences by country.
spatial_data |>
...
# Number of occurences by source
spatial_data |>
  mutate(Age.3 = case_when(Age.3 == "DanSelCombined" ~ "Danian-Selandian", T ~ Age.3)) |> 
  mutate(Age.3 = factor(Age.3, levels = c("Campanian","Maastrichtian","Danian-Selandian","Thanetian"))) |>
...
# Numer of occurences by species.
spatial_data |> 
  mutate(Age.3 = case_when(Age.3 == "DanSelCombined" ~ "Danian-Selandian", T ~ Age.3)) |> 
  mutate(Age.3 = factor(Age.3, levels = c("Campanian","Maastrichtian","Danian-Selandian","Thanetian"))) |>
  group_by(Species) |> 
...
# Function used to 1) assign occurences to cells based on age and 2) filter by minimum occurence.
cell_preparation <- function(data,age,hexa,occurrence) {
  ...
  # Merge centroids to master-dataframe.
